...
enum HotkeyFormat {
    static func combo(from event: NSEvent) -> String? {
        ...
        guard let key = keyName(event.keyCode) else { return nil }
        parts.append(key)
...
public struct HotkeyRecorderView: View {
    ...
    private func handleKeyEvent(_ event: NSEvent) {
        ...
        let mods = event.modifierFlags.intersection([.command, .shift, .control, .option])
        guard !mods.isEmpty, let comboString = HotkeyFormat.combo(from: event) else { return }
        combo = comboString
        stopRecording()
        onSave()
    }
