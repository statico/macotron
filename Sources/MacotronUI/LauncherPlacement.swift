...
enum LauncherPlacement {
    ...
    /// Frame for the launcher inside `visible`. Pass `pinTop` (current maxY) when resizing
    /// so the search field stays put and the list grows downward.
    static func frame(height: CGFloat, visible: CGRect, pinTop: CGFloat?) -> CGRect {
        ...
        let desiredTop = pinTop ?? (visible.maxY - margin)
