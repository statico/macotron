...
macotron.keyboard.on("ctrl+opt+left", () =>
  tile({ x: 0, y: 0, w: 0.5, h: 1 })
);

macotron.keyboard.on("ctrl+opt+right", () =>
  tile({ x: 0.5, y: 0, w: 0.5, h: 1 })
);

macotron.keyboard.on("ctrl+opt+up", () =>
  tile({ x: 0, y: 0, w: 1, h: 0.5 })
);

macotron.keyboard.on("ctrl+opt+down", () =>
  tile({ x: 0, y: 0.5, w: 1, h: 0.5 })
);
