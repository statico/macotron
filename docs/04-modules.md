...
**Launcher:** `macotron.launcher.set(id, items)` replaces that plugin's extra rows in the quick launcher. Each item is `{ id, title, subtitle?, app?, sfSymbol?, kind?, onClick }`. `macotron.launcher.query(id, fn)` answers the typed text instead; `fn` may return rows or a promise of rows, and the launcher shows late rows when the promise settles rather than holding up the keystroke. `app` is a bundle ID (Notes uses `com.apple.Notes`). An empty query shows only starred items (⌘S). Typing filters plugin rows with apps and commands.
...
**Calendar:** `upcoming({ hours })` resolves with `{ id, title, start, end, allDay, location, calendar }[]`. Times are epoch ms.
