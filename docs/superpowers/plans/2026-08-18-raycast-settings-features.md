# Raycast-Style Settings Features Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add four Raycast-inspired features to Macotron Settings: launch at login, launcher text-size boost, an appearance switcher (System/Dark/Light for Macotron's own windows, independent of the system), and a master-detail Plugins tab with per-plugin enable/disable.
...
**Interpretation note:** "Appearance switcher per app" = Macotron's own appearance override (System / Dark / Light), exactly like the Raycast General settings pane in the reference screenshot. It does not change other apps.
