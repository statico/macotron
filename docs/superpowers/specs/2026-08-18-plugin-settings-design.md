...
- Extend the existing `macotron.module({ options })` API (Raycast-style declarative prefs).
...
"pluginSettings": {
  "chat.js": {
    ...
    "notesFile": "/Users/ian/notes.md",
    "workspace": "/Users/ian/dev/project"
  }
}
```
...
Raycast extension `preferences` (declarative types + host form + password type). VS Code `contributes.configuration`. Alfred workflow config fields.
